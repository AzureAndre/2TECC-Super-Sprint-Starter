...
#func _on_track_zone_area_entered(area: Area2D) -> void:
	#print("Crash!!")
	#if area is Car: print("Car Crash")
	#if area is Car: area.hit_boundry()
	#if area is Car: area.hit_boundry(get_direction_to_path(area.position))
