...
#@export var steer_strength: float = 8.0
#@export var min_steer_factor: float = 1
...
#func get_steer_factor() -> float:
	...
	#return clampf(1-(_velocity/max_speed), min_steer_factor, steer_strength)
	##return clampf(1-pow(_velocity/max_speed),2.0), min_steer_factor, steer_strength)
...
#
	...
	##_bounce_tween.tween_property(self, "rotation_degrees", rotation_degrees + 360.0 + randf_range(-45.0, 45.0), bounce_time)
	##_bounce_tween.set_parallel(false)
	##_bounce_tween.finished.connect(bounce_done)
	#position += -transform.x * bounce_force
	##await get_tree().create_timer(bounce_force).timeout
	#set_physics_process(true)

#func hit_boundry() -> void:
	#bounce()
#func hit_boundry(dir_to_path: Vector2) -> void:
